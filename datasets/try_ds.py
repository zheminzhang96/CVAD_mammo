...
def get_try_data(laterality):
    ...
    train_ids, val_ids = train_test_split(df_view['PatientID'].unique(),test_size=0.30,random_state=1996)
    ...
    train_df_cc = df_view[df_view['split']=='train']
    val_df_cc = df_view[df_view['split']=='val']
    #test_df_cc = df_view[df_view['split']=='test']
    ...
    return train_df_cc[0:260], val_df_cc[0:60]#, test_df_cc[0:4]
...
# def image_process(df_view, imgSize, i): # i will be in the range of len(patient_id)
...
#     if check_laterality(img_r, 'R') == True:
#         img_crop_r = image_preprocessing.segment_img(img_r)     
#     else:
#         img_crop_r = image_preprocessing.segment_img(img_l)
        
#     if check_laterality(img_l, 'L') == True:
#         img_crop_l = image_preprocessing.segment_img(img_l)
#     else:
#         img_crop_l = image_preprocessing.segment_img(img_r)
#     #print(img.shape)
#     img_conct = image_preprocessing.stitch_images(img_crop_r, img_crop_l)
